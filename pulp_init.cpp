...
void pulp_init_bfs_pull(dist_graph_t* g, mpi_data_t* comm, queue_data_t* q,
                        pulp_data_t* pulp) {
#pragma omp parallel
  {
#pragma omp for
  ...
  if (procid == 0) {
    ...
    for (int32_t i = 1; i < pulp->num_parts; ++i)
      if (roots[i] <= roots[i - 1]) roots[i] = roots[i - 1] + 1;

    if (debug) {
      printf("BFS Init Roots ");
      for (int32_t i = 0; i < pulp->num_parts; ++i) printf("%lu ", roots[i]);
      printf("\n");
    }
  }
...
#pragma omp for schedule(guided) nowait
      for (uint64_t vert_index = 0; vert_index < g->n_local; ++vert_index) {
        if (pulp->local_parts[vert_index] >= 0) continue;
...
#pragma omp single
      {
        ...
        if (debug)
          printf("Task %d send_size %lu global_size %li\n", procid,
                 temp_send_size, comm->global_queue_size);
      }
...
void pulp_init_bfs_max(dist_graph_t* g, mpi_data_t* comm, queue_data_t* q,
  ...
  int64_t max_part_size =
      int64_t((double)(g->n / pulp->num_parts) * sqrt((double)pulp->num_parts));
...
#pragma omp for schedule(guided) nowait
      for (uint64_t vert_index = 0; vert_index < g->n_local; ++vert_index) {
        ...
        for (uint64_t j = 0; j < out_degree; ++j) {
          ...
          int32_t part_out = pulp->local_parts[out_index];
          if (part_out >= 0 &&
              pulp->part_sizes[part_out] + pulp->part_size_changes[part_out] <
                  max_part_size) {
            pulp->local_parts[vert_index] = part_out;
            new_part = part_out;
            break;
...
void pulp_init_label_prop_weighted(dist_graph_t* g, mpi_data_t* comm,
  ...
  if (debug) {
    printf("Task %d pulp_init_label_prop_weighted() start\n", procid);
  }
...
#pragma omp single
  ...
  if (debug) {
    printf("Task %d pulp_init_label_prop_weighted() success\n", procid);
  }
}
      ...
      for (uint64_t vert_index = 0; vert_index < g->n_local; ++vert_index) {
        ...
        if (max_part != part) {
          ...
          pulp->part_size_changes[part] - 1 > 0
              ? new_size =
                    pulp->part_sizes[part] + pulp->part_size_changes[part] - 1
              : new_size =
                    (int64_t)((double)pulp->part_sizes[part] +
                              multiplier * pulp->part_size_changes[part] - 1);
