        ...
        for (int32_t p = 0; p < pulp->num_parts; ++p) {
          tp.part_weights[p] =
              vert_balance * pulp->avg_size /
                  ((double)pulp->part_sizes[p] +
                   multiplier * (double)pulp->part_size_changes[p]) -
              1.0;
          ...
          for (uint64_t j = 0; j < out_degree; ++j) {
            ...
            if (out_index >= g->n_local) {
              tp.part_counts[part_out] +=
                  g->ghost_degrees[out_index - g->n_local];
            } else {
        ...
        for (uint64_t vert_index = 0; vert_index < g->n_local; ++vert_index) {
          ...
          if (max_part != part) {
            /*printf("%d %d - %lu to %d (%li + %li) from %d (%li + %li), %f\n",
              procid, omp_get_thread_num(), g->local_unmap[vert_index],
              max_part, pulp->part_sizes[max_part],
              pulp->part_size_changes[max_part], part, pulp->part_sizes[part],
              pulp->part_size_changes[part], max_val);*/
...
#pragma omp for schedule(guided) reduction(+ : num_swapped_2) nowait
        for (uint64_t vert_index = 0; vert_index < g->n_local; ++vert_index) {
          ...
          if (max_part != part) {
            ...
            /*printf("%d %d - %lu to %d (%li + %li) from %d (%li + %li) -- %li
              %li\n", procid, omp_get_thread_num(), g->local_unmap[vert_index],
              max_part, pulp->part_sizes[max_part],
              pulp->part_size_changes[max_part], part, pulp->part_sizes[part],
              pulp->part_size_changes[part], new_size,
              (int64_t)(pulp->avg_size*vert_balance));*/
