...
int load_graph_edges_32(char* input_filename, graph_gen_data_t* ggi,
  ...
  uint64_t read_offset_start =
      procid * 2 * sizeof(uint32_t) * (nedges_global / nprocs);
  uint64_t read_offset_end =
      (procid + 1) * 2 * sizeof(uint32_t) * (nedges_global / nprocs);
  ...
  if (debug) {
    printf(
        "Task %d, read_offset_start %ld, read_offset_end %ld, nedges_global "
        "%ld, nedges: %ld\n",
...
int exchange_edges(graph_gen_data_t* ggi, mpi_data_t* comm) {
  ...
  MPI_Alltoall(temp_sendcounts, 1, MPI_UINT64_T, temp_recvcounts, 1,
  ...
  MPI_Allreduce(MPI_IN_PLACE, &num_comms, 1, MPI_UINT64_T, MPI_MAX,
                MPI_COMM_WORLD);

  if (debug)
    printf(
        "Task %d exchange_edges() num_comms %lu total_send %lu total_recv "
        "%lu\n",
  ...
  for (uint64_t c = 0; c < num_comms; ++c) {
    ...
    if (sendbuf == NULL) {
      fprintf(stderr,
              "Task %d Error: exchange_out_edges(), unable to allocate comm "
              "buffers",
              procid);
      MPI_Abort(MPI_COMM_WORLD, 1);
    }
  ...
  if (verbose) {
    elt = omp_get_wtime() - elt;
    printf(
        "Task %d exchange_out_edges() sent %lu, recv %lu, m_local_edges %lu, "
        "%9.6f (s)\n",
