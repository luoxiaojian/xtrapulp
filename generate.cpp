...
int generate_rand_out_edges(graph_gen_data_t* ggi, uint64_t num_verts,
  ...
  if (debug) {
    printf("Task %d generate_rand_out_edges() start\n", procid);
  }
  ...
  if (debug) {
    printf(
        "Task %d, n %li, m %li, n_offset %li, n_local %li, m_local_read %li\n",
  ...
  for (uint64_t i = 0; i < ggi->n_local; ++i) {
    for (uint64_t j = 0; j < edges_per_vert; ++j) {
      ...
      while (v1 == v2) v2 = xs1024star_next(&xs) % ggi->n;
...
#pragma omp parallel for
  ...
  if (debug) {
    printf("Task %d generate_rand_out_edges() success\n", procid);
  }
  return 0;
}
...
int generate_rmat_out_edges(graph_gen_data_t* ggi, uint64_t num_verts,
                            uint64_t edges_per_vert, bool offset_vids) {
  if (debug) {
    printf("Task %d generate_rmat_out_edges() start\n", procid);
  }
...
#pragma omp parallel for
  ...
  if (debug) {
    printf("Task %d generate_rmat_out_edges() success\n", procid);
  }
  return 0;
}
...
int generate_hd_out_edges(graph_gen_data_t* ggi, uint64_t num_verts,
                          uint64_t edges_per_vert, bool offset_vids) {
  if (debug) {
    printf("Task %d generate_mesh_out_edges() start\n", procid);
  }
  ...
  for (uint64_t i = ggi->n_offset; i < (ggi->n_offset + ggi->n_local); ++i) {
    ...
    for (uint64_t j = 0; j < edges_per_vert; ++j) {
      while (xs1024star_next_real(&xs) < 0.5 || cur == i) ++cur;
...
#pragma omp parallel for
  ...
  if (debug) {
    printf("Task %d generate_mesh_out_edges() success\n", procid);
  }
  return 0;
}
