...
int pulp_vec(dist_graph_t* g, mpi_data_t* comm, queue_data_t* q,
             pulp_data_t* pulp, uint64_t outer_iter, uint64_t balance_iter,
             uint64_t refine_iter, double vert_balance, double edge_balance) {
  if (debug) {
...
#pragma omp parallel default(shared)
  {
    ...
    for (uint64_t cur_outer_iter = 0; cur_outer_iter < outer_iter;
         ++cur_outer_iter) {
#pragma omp single
      {
      ...
      for (uint64_t cur_bal_iter = 0; cur_bal_iter < balance_iter;
           ++cur_bal_iter) {
        for (int32_t p = 0; p < pulp->num_parts; ++p) {
          tp.part_weights[p] =
              vert_balance * pulp->avg_size / (double)pulp->part_sizes[p] - 1.0;
          tp.part_edge_weights[p] = pulp->max_e * pulp->avg_edge_size /
                                        (double)pulp->part_edge_sizes[p] -
                                    1.0;
          tp.part_cut_weights[p] = pulp->max_c * pulp->avg_cut_size /
                                       (double)pulp->part_cut_sizes[p] -
                                   1.0;
          if (tp.part_weights[p] < 0.0) tp.part_weights[p] = 0.0;
          if (tp.part_edge_weights[p] < 0.0) tp.part_edge_weights[p] = 0.0;
          if (tp.part_cut_weights[p] < 0.0) tp.part_cut_weights[p] = 0.0;
        }
...
#pragma omp for schedule(guided) reduction(+ : num_swapped_1) nowait
        for (uint64_t vert_index = 0; vert_index < g->n_local; ++vert_index) {
          ...
          for (int32_t p = 0; p < pulp->num_parts; ++p) tp.part_counts[p] = 0.0;
          ...
          for (int32_t p = 0; p < pulp->num_parts; ++p) {
            ...
            if (tp.part_weights[p] > 0.0 && tp.part_edge_weights[p] > 0.0 &&
              ...
              tp.part_counts[p] *=
                  (tp.part_edge_weights[p] * pulp->weight_exponent_e *
                   tp.part_cut_weights[p] * pulp->weight_exponent_c);
            else
...
#pragma omp atomic
            ...
            tp.part_weights[part] =
                vert_balance * pulp->avg_size /
                    ((double)pulp->part_sizes[part] +
                     multiplier * (double)pulp->part_size_changes[part]) -
                1.0;
            tp.part_weights[max_part] =
                vert_balance * pulp->avg_size /
                    ((double)pulp->part_sizes[max_part] +
                     multiplier * (double)pulp->part_size_changes[max_part]) -
                1.0;

            tp.part_edge_weights[part] =
                pulp->max_e * pulp->avg_edge_size /
                    ((double)pulp->part_edge_sizes[part] +
                     multiplier * (double)pulp->part_edge_size_changes[part]) -
                1.0;
            tp.part_edge_weights[max_part] =
                pulp->max_e * pulp->avg_edge_size /
                    ((double)pulp->part_edge_sizes[max_part] +
                     multiplier *
                         (double)pulp->part_edge_size_changes[max_part]) -
                1.0;
            ...
            tp.part_cut_weights[part] =
                pulp->max_c * avg_cut_size /
                    ((double)pulp->part_cut_sizes[part] +
                     multiplier * (double)pulp->part_cut_size_changes[part]) -
                1.0;
            tp.part_cut_weights[max_part] =
                pulp->max_c * avg_cut_size /
                    ((double)pulp->part_cut_sizes[max_part] +
                     multiplier *
                         (double)pulp->part_cut_size_changes[max_part]) -
                1.0;

            if (tp.part_weights[part] < 0.0) tp.part_weights[part] = 0.0;
...
#pragma omp single
        {
          ...
          MPI_Allreduce(MPI_IN_PLACE, pulp->part_edge_size_changes,
                        pulp->num_parts, MPI_INT64_T, MPI_SUM, MPI_COMM_WORLD);
          MPI_Allreduce(MPI_IN_PLACE, pulp->part_cut_size_changes,
                        pulp->num_parts, MPI_INT64_T, MPI_SUM, MPI_COMM_WORLD);
          MPI_Allreduce(MPI_IN_PLACE, &pulp->cut_size_change, 1, MPI_INT64_T,
          ...
          //  i, pulp->part_sizes[i], pulp->part_edge_sizes[i],
          ...
          for (int32_t p = 0; p < pulp->num_parts; ++p) {
            ...
            if ((double)pulp->part_sizes[p] / pulp->avg_size > pulp->max_v)
              pulp->max_v = (double)pulp->part_sizes[p] / pulp->avg_size;
            if ((double)pulp->part_edge_sizes[p] / pulp->avg_edge_size >
                pulp->max_e)
              pulp->max_e =
                  (double)pulp->part_edge_sizes[p] / pulp->avg_edge_size;
            if ((double)pulp->part_cut_sizes[p] / pulp->avg_cut_size >
                pulp->max_c)
              pulp->max_c =
                  (double)pulp->part_cut_sizes[p] / pulp->avg_cut_size;
            if (pulp->part_cut_sizes[p] > pulp->max_cut)
              pulp->max_cut = pulp->part_cut_sizes[p];
          }
          if (pulp->max_e < edge_balance) {
          ...
          cur_iter += 1.0;
          // multiplier =
          // (double)pulp->num_parts*(1.0-(double)cur_iter/(double)tot_iter)+1.0*pulp->num_parts*(double)cur_iter/((double)tot_iter*2.0);
          ...
          //    pulp->max_v, pulp->max_e, pulp->max_c, pulp->cut_size,
          //    pulp->max_cut);
          // part_eval(g, pulp);
          // printf("\n\n\n\n");
        }
...
#pragma omp single
      ...
      for (uint64_t cur_ref_iter = 0; cur_ref_iter < refine_iter;
           ++cur_ref_iter) {
        ...
        for (uint64_t vert_index = 0; vert_index < g->n_local; ++vert_index) {
          ...
          if (max_part != part) {
            ...
            pulp->part_size_changes[max_part] + 1 < 0
                ? new_size = pulp->part_sizes[max_part] +
                             pulp->part_size_changes[max_part] + 1
                : new_size = (int64_t)(
                      (double)pulp->part_sizes[max_part] +
                      multiplier * (double)pulp->part_size_changes[max_part] +
                      1.0);
            ...
            pulp->part_edge_size_changes[max_part] + out_degree < 0
                ? new_edge_size = pulp->part_edge_sizes[max_part] +
                                  pulp->part_edge_size_changes[max_part] +
                                  out_degree
                : new_edge_size = (int64_t)(
                      (double)pulp->part_edge_sizes[max_part] +
                      multiplier *
                          (double)pulp->part_edge_size_changes[max_part] +
                      (double)(out_degree));
            ...
            pulp->part_cut_size_changes[part] < 0
                ? new_cut_size = pulp->part_cut_sizes[part] +
                                 pulp->part_cut_size_changes[part] +
                                 2 * part_count - out_degree
                : new_cut_size = (int64_t)(
                      (double)pulp->part_cut_sizes[part] +
                      multiplier * (double)pulp->part_cut_size_changes[part] +
                      2.0 * (double)part_count - (double)(out_degree));

            pulp->part_cut_size_changes[max_part] < 0
                ? new_cut_size = pulp->part_cut_sizes[max_part] +
                                 pulp->part_cut_size_changes[max_part] +
                                 out_degree - 2 * max_count
                : new_cut_size = (int64_t)(
                      (double)pulp->part_cut_sizes[max_part] +
                      multiplier *
                          (double)pulp->part_cut_size_changes[max_part] +
                      (double)(out_degree)-2.0 * (double)max_count);
...
#pragma omp single
        {
          ...
          for (int32_t p = 0; p < pulp->num_parts; ++p) {
            ...
            /*if ((double)pulp->part_sizes[p] / pulp->avg_size > pulp->max_v)
              pulp->max_v = (double)pulp->part_sizes[p] / pulp->avg_size;
            if ((double)pulp->part_edge_sizes[p] / pulp->avg_edge_size >
            pulp->max_e) pulp->max_e = (double)pulp->part_edge_sizes[p] /
            pulp->avg_edge_size; if ((double)pulp->part_cut_sizes[p] /
            pulp->avg_cut_size > pulp->max_c) pulp->max_c =
            (double)pulp->part_cut_sizes[p] / pulp->avg_cut_size;*/
            if (pulp->part_cut_sizes[p] > pulp->max_cut)
              pulp->max_cut = pulp->part_cut_sizes[p];
          }
...
int pulp_vec_weighted(dist_graph_t* g, mpi_data_t* comm, queue_data_t* q,
  ...
  if (debug) {
    printf("Task %d pulp_vec_weighted() start\n", procid);
  }
  double elt = 0.0;
  if (verbose) {
    MPI_Barrier(MPI_COMM_WORLD);
...
#pragma omp single
  ...
  if (debug) {
    printf("Task %d pulp_vec_weighted() success\n", procid);
  }
