...
inline void update_sendcounts_thread(dist_graph_t* g, thread_comm_t* tc,
                                     uint64_t vert_index);
inline void update_vid_data_queues(dist_graph_t* g, thread_comm_t* tc,
                                   mpi_data_t* comm, uint64_t vert_index,
                                   int32_t* data);
...
inline void add_vid_data_to_send(thread_comm_t* tc, mpi_data_t* comm,
                                 uint64_t vertex_id, int32_t data_val,
                                 int32_t send_rank);
...
inline void exchange_verts(dist_graph_t* g, mpi_data_t* comm, queue_data_t* q) {
  ...
  MPI_Allreduce(&task_queue_size, &comm->global_queue_size, 1, MPI_UINT64_T,
  ...
  for (uint64_t c = 0; c < num_comms; ++c) {
    ...
    MPI_Alltoall(comm->sendcounts, 1, MPI_INT32_T, comm->recvcounts, 1,
    ...
    comm->sendbuf_vert =
        (uint64_t*)malloc((uint64_t)(cur_send + 1) * sizeof(uint64_t));
    ...
    MPI_Alltoallv(comm->sendbuf_vert, comm->sendcounts, comm->sdispls,
                  MPI_UINT64_T, q->queue_next + q->next_size + sum_recv,
...
inline void exchange_vert_data(dist_graph_t* g, mpi_data_t* comm,
  ...
  MPI_Alltoall(comm->sendcounts_temp, 1, MPI_UINT64_T, comm->recvcounts_temp, 1,
  ...
  for (uint64_t c = 0; c < num_comms; ++c) {
    ...
    int32_t cur_recv = comm->rdispls[nprocs - 1] + comm->recvcounts[nprocs - 1];
    uint64_t* buf_v =
        (uint64_t*)malloc((uint64_t)(cur_send) * sizeof(uint64_t));
    ...
    MPI_Alltoallv(buf_v, comm->sendcounts, comm->sdispls, MPI_UINT64_T,
                  comm->recvbuf_vert + sum_recv, comm->recvcounts,
                  comm->rdispls, MPI_UINT64_T, MPI_COMM_WORLD);
    MPI_Alltoallv(buf_d, comm->sendcounts, comm->sdispls, MPI_INT32_T,
...
inline void update_vid_data_queues(dist_graph_t* g, thread_comm_t* tc,
  ...
  for (uint64_t j = 0; j < out_degree; ++j) {
    ...
    if (out_index >= g->n_local) {
      ...
      if (!tc->v_to_rank[out_rank]) {
        tc->v_to_rank[out_rank] = true;
        add_vid_data_to_send(tc, comm, g->local_unmap[vert_index], data,
                             out_rank);
      }
    }
  }
