        ...
        for (uint64_t vert_index = 0; vert_index < g->n_local; ++vert_index) {
          ...
          for (int32_t p = 0; p < pulp->num_parts; ++p) {
            if (tp.part_weights[p] > 0.0 && tp.part_edge_weights[p] > 0.0)
              tp.part_counts[p] *=
                  (tp.part_weights[p] * tp.part_edge_weights[p] *
                   pulp->weight_exponent_e);
            else
          ...
          if (max_part != part) {
            ...
            new_size =
                pulp->part_size_changes[max_part] + 1 < 0
                    ? pulp->part_sizes[max_part] +
                          pulp->part_size_changes[max_part] + 1
                    : (int64_t)((double)pulp->part_sizes[max_part] +
                                multiplier *
                                    (double)pulp->part_size_changes[max_part] +
                                1.0);

            new_edge_size =
                pulp->part_edge_size_changes[max_part] + out_degree < 0
                    ? pulp->part_edge_sizes[max_part] +
                          pulp->part_edge_size_changes[max_part] + out_degree
                    : (int64_t)(
                          (double)pulp->part_edge_sizes[max_part] +
                          multiplier *
                              (double)pulp->part_edge_size_changes[max_part] +
...
#pragma omp single
        {
          ...
          for (int32_t p = 0; p < pulp->num_parts; ++p) {
            ...
            // if ((double)pulp->part_edge_sizes[p] / pulp->avg_edge_size >
            // pulp->max_e)
            // pulp->max_e = (double)pulp->part_edge_sizes[p] /
            // pulp->avg_edge_size;
          }
...
int pulp_ve_weighted(dist_graph_t* g, mpi_data_t* comm, queue_data_t* q,
  ...
  if (debug) {
    printf("Task %d pulp_ve_weighted() start\n", procid);
  }
  double elt = 0.0;
  if (verbose) {
    MPI_Barrier(MPI_COMM_WORLD);
...
#pragma omp single
  ...
  if (debug) {
    printf("Task %d pulp_ve_weighted() success\n", procid);
  }
